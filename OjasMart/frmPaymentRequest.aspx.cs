...
namespace OjasMart
{
    public partial class frmPaymentRequest : System.Web.UI.Page
    {
        ...
        public void InsertMethod(string txnId)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(txnId))
                {
                    objp.Action = "1";
                    objp.txnId = txnId;
                    DataTable dt = objL.GetOnlineWalletRechargeInfo(objp, "proc_getOnlineWalletRechargeInfo");
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        BookingRequest(dt.Rows[0]["TransactionId"].ToString(), dt.Rows[0]["TransactionId"].ToString(), dt.Rows[0]["RechargeAmount"].ToString(), dt.Rows[0]["Name"].ToString(), dt.Rows[0]["EmailAddress"].ToString(), dt.Rows[0]["MobileNo"].ToString(), dt.Rows[0]["PInfo"].ToString(), "../frmResponceHandling.aspx", "../frmResponceHandling.aspx", txnid1);
                    }
                    else
                    {
